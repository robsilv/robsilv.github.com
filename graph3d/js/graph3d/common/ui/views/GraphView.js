...
{	
	...
	p._init = function _init()
	{
		...
		this._gridLineColor = 0x222222;
		this._gridLineOpacity = 1;
		this._gridLineWidth = 5;
		
		this._axisLineWidth = 5;
		
		this._defaultTextSize = 16;
		this._axisLength = 1000;
		
		this._axisStyles = [{ lineColor:0xff0000, endValue:this._axisLength},
	...
	*/
	...
	p._render = function _render() 
	{
		...
		if ( intersects.length > 0 ) {
			...
			if ( this.INTERSECTED != intersects[ 0 ].object ) {
				...
				if ( this.INTERSECTED ) this.INTERSECTED.material.opacity = 1;
			...
			if ( this.INTERSECTED ) this.INTERSECTED.material.opacity = 0.8;
	...
	{
		// lines
		var line = new THREE.Line( geom, new THREE.LineBasicMaterial( { color: color.getHex(), opacity: 1 } ) );
	...
	{		
		...
		for ( var i = 0; i < 3; i ++ )
		{
			var axisObj = this._axisStyles[i];
			var geometry = new THREE.Geometry();
			geometry.vertices.push( new THREE.Vector3( 0, 0, 0 ) );
			geometry.vertices.push( new THREE.Vector3( 0, 0, 0 ) );

			var line = new THREE.Line( geometry, new THREE.LineBasicMaterial( { color: axisObj.lineColor, opacity: 1, linewidth: this._axisLineWidth } ) );
	...
	{		
		...
		for ( var i = 1; i <= numStepsA; i ++ ) 
		{
			...
			var line = new THREE.Line( geometry, new THREE.LineBasicMaterial( { color: this._gridLineColor, opacity: this._gridLineOpacity, linewidth: this._gridLineWidth } ) );
